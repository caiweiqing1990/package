...
#define PATH_LEN			256
#define URL_LEN				256
#define CON_TIME_OUT		30

#define MAX_RECV_SIZE    	1440	//硬件单包最大的接收字节数
char g_route[URL_LEN]={0};
char g_host[URL_LEN]={0};
char g_ip[URL_LEN]={0};				//ip/域名
char g_port[16];

char g_buf_send[4*1024]={0};		//发送数据暂存区
char g_buf_recv[10*1024]={0};		//接收数据暂存区
...
int Socket_Connect(char* routename,char *ip,char *port)
{
    struct sockaddr_in addr;
	struct timeval timeo = {CON_TIME_OUT, 0};
    int sockfd;
    int len;
...
int HTTP_GetFileSize(int sockfd,char *path)
{
	...
	if(strstr(buf_recv_tmp, "OK") == NULL)
	{
		return -1;
	}
...
int HTTP_GetFile(int sockfd,char *path,int filelength,int download_size,char *filebuf)
{
    ...
    for(i=download_index;i<count;i++)
    {
        ...
        if(ret == 0 )//服务端断开连接
        {
            sockfd = Socket_Connect(g_route, g_ip, g_port);
            i--;
            continue;
        }
        /*提取协议体数据,保存在filebuf中*/
		...
		{
			datalen = (ret - (p - g_buf_recv) - 4);
			memcpy(filebuf + offset, p+4, datalen);
			offset += datalen;
...
int HTTP_DownloadFile(char *url,char *save_path)
{
    ...
    if(filesize == -1)
        return -1;
    //malloc分配存储文件空间
    filebuf = (char *)malloc(filesize+1024);
    if(filebuf == NULL)
    {
    ...
    else
    {
		...
		#endif
		Save_File(filebuf,ret,save_path);
		free(filebuf);
		...
		else//部分下载完成
		{
			...
			//保存断点信息
			Save_Breakpoint(url,save_path,ret+download_size,NULL);
			return -2;
		}
    }
}
...
{	
	if(argc != 4)
	{
		printf("%s <RouteName> <SaveFileName> <Url>\n", argv[0]);
		return -1;
	}

	strcpy(g_route, argv[1]);
	HTTP_DownloadFile(argv[3], argv[2]);
	return 0;
}
