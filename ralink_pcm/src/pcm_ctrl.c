...
void pcm_exit(void)
{
	kfree(ppcm_config->mmapbuf_r);
	kfree(ppcm_config->mmapbuf_t);
	pcm_close();
...
void pcm_tx_task(unsigned long pData)
{
	...
	{
		u32 dmach = ptrpcm_config->unmask_ch;
		for (ch = 0; ch < 16; ch++)
		{
			if (dmach& (1<<ch))
			{
				MSG("do unmask ch%d in tx_task\n",ch);
				GdmaUnMaskChannel(ch);
			}	
		}
		ptrpcm_config->unmask_ch = 0;	
...
void pcm_unmask_isr(u32 dma_ch)
{
	...
	MSG("umisr c=%d\n",dma_ch);
	ptrpcm_config->unmask_ch |= (1<<dma_ch);
...
#if 1
			if(ptrpcm_config->data_offset_t == 0)
			{
				memset(ptrpcm_config->mmapbuf_t, 0, ptrpcm_playback->playback_max_size * 2);
			}
